...
const Navbar = () => {
    ...
    const {setShowSearch, CartLength, token, setToken, setCartItems, navigate} = useContext(ShopContext)
    ...
    <div className='flex items-center justify-between py-5 font-medium'>
        ...
        <div className="flex items-center gap-6">
            ...
            <Link to={'/cart'} className="relative">
                ...
                <p className='absolute right-[-5px] bottom-[-5px] text-white bg-black aspect-square rounded-full text-[8px] w-4 text-center leading-4'>{CartLength()}</p>
            </Link>
        ...
        <div className={`absolute top-0 right-0 bottom-0 overflow-hidden transition-all bg-white ${visible ? 'w-full' : 'w-0'}`}>
            <div className='flex flex-col text-gray-600'>
                ...
                <NavLink onClick={()=>setVisible(false)} className="py-2 pl-6 border border-[#e5e7eb]" to={'/'}>HOME</NavLink>
                <NavLink onClick={()=>setVisible(false)} className="py-2 pl-6 border border-[#e5e7eb]" to={'/collection'}>COLLECTION</NavLink>
                <NavLink onClick={()=>setVisible(false)} className="py-2 pl-6 border border-[#e5e7eb]" to={'/about'}>ABOUT</NavLink>
                <NavLink onClick={()=>setVisible(false)} className="py-2 pl-6 border border-[#e5e7eb]" to={'/contact'}>CONTACT</NavLink>
            </div>
        </div>
    </div>
  )
